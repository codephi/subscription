...
impl McpConfig {
    fn from_env() -> Self {
        let enabled = cfg!(feature = "mcp") && parse_env_bool("MCP_ENABLED").unwrap_or(false);
...
mod tests {
    ...
    #[test]
    #[cfg(not(feature = "mcp"))]
    fn mcp_endpoint_url_is_absent_without_mcp_feature() {
        let _guard = ENV_LOCK.lock().expect("env lock");
        let _database_url = EnvVarGuard::set("DATABASE_URL", "postgres://localhost/template");
        ...
        assert_eq!(config.mcp_endpoint_url(), None);
    }
